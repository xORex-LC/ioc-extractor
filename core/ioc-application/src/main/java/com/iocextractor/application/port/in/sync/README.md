...
- `reconcile` materializes missing pairs без remote IO; `publish` обрабатывает
  уже известное retryable ledger state без profile scan.
- Driving adapters, которым нужен ручной полный цикл, вызывают `reconcile` перед
  `publish` явно.
