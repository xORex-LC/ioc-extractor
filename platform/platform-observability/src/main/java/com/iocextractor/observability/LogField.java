...
public enum LogField {
    ...
    IOC_DECISION_KIND("ioc.decision.kind", "Pipeline processing step that made a per-item decision."),
    IOC_DECISION_OUTCOME("ioc.decision.outcome", "Stable outcome of a per-item pipeline decision."),
    IOC_DECISION_RULE("ioc.decision.rule", "Rule or source marker selected by a pipeline decision."),
    IOC_DECISION_PATTERN("ioc.decision.pattern", "Pattern or predicate set evaluated by a pipeline decision."),
    IOC_DECISION_RESULT("ioc.decision.result", "Compact materialized result of a pipeline decision."),
    IOC_ITEM_IDENTITY("ioc.item.identity", "Safe short identity of the item involved in a pipeline decision."),
    IOC_ITEM_VALUE("ioc.item.value", "TRACE-only item value with query-like data redacted."),
    IOC_INDICATOR_TYPE("ioc.indicator.type", "IOC type involved in a pipeline decision."),
    IOC_SPAN_START("ioc.span.start", "Inclusive source-text offset of a matched item."),
    IOC_SPAN_END("ioc.span.end", "Exclusive source-text offset of a matched item."),
    ...
    IOC_COMPLETION_STATUS("ioc.completion.status", "Typed completion status of a structurally finished processing run."),
    IOC_DIAGNOSTIC_TOTAL("ioc.diagnostic.total", "Total diagnostics observed during the run, including suppressed occurrences."),
    IOC_DIAGNOSTIC_SUPPRESSED("ioc.diagnostic.suppressed", "Diagnostics suppressed by the per-run retention budget."),
    IOC_DIAGNOSTIC_FATAL_COUNT("ioc.diagnostic.count.fatal", "FATAL diagnostics observed during the run."),
    IOC_DIAGNOSTIC_ERROR_COUNT("ioc.diagnostic.count.error", "ERROR diagnostics observed during the run."),
    IOC_DIAGNOSTIC_WARN_COUNT("ioc.diagnostic.count.warn", "WARN diagnostics observed during the run."),
    IOC_DIAGNOSTIC_INFO_COUNT("ioc.diagnostic.count.info", "INFO diagnostics observed during the run."),
    IOC_DIAGNOSTIC_DEBUG_COUNT("ioc.diagnostic.count.debug", "DEBUG diagnostics observed during the run."),
    IOC_DIAGNOSTIC_TRACE_COUNT("ioc.diagnostic.count.trace", "TRACE diagnostics observed during the run."),
