...
и изоляцию ошибок per source/target; следующий tick является macro retry. При активном
daemon publish событие `SliceCompleted` дополнительно разворачивается в per-target
команды через keyed executor по endpoint. Это fast-path, а не durable broker:
источником восстановления остаются `publish_ledger` и reconcile.
