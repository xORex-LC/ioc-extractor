...
| Порт | Тип | Адаптеры |
|---|---|---|
| `DiagnosticSink` | out | `ReportDiagnosticSink`, `CollectingSink` (накопитель), `LoggingDiagnosticSink` через bridge-модуль |
...
1. Ядро: `Diagnostic`, `DiagnosticCode` + первые каталоги, `Result`/`Notification`,
   порты `DiagnosticSink`/`DiagnosticRenderer` + `CollectingSink`.
2. `FailurePolicy` (collect-and-continue) + отчёт по прогону.
3. Карта ошибок (генерация из каталога); i18n шаблонов (опц.).
