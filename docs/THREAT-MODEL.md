...
| **T/D/E** — hostile managed CSV либо превышение source authority | oversized field/row, ambiguous schema, formula payload или contract пытается изменить запрещённый artifact/field | `SEC-INP-4`: private bounded snapshot, strict streaming parse/exact-one recognition, formula policy, versioned mapping и startup-compiled source authority ceiling; `SEC-DATA-2` не допускает partial canonical apply | per-file cryptographic producer signature не поддерживается; доступ на запись в inbox остаётся trust decision источника |
...
| **T** — подмена fetched-контента | MITM или подменённый файл на шаре | secure default `encryption: required` допускает только SMB3 и fail-closed проверяет effective session encryption до первого share request; downstream повторно применяет input controls границы 1 | `preferred`/`disabled` являются явным принятием доверия к сети; per-file signature источника не поддерживается |
| **T/R** — гонка producer/consumer при managed import | файл меняется после listing либо disconnect происходит после claim rename | `SEC-DATA-2`: evidence compare, server-side rename в private namespace, read без concurrent write sharing, before/after evidence, hashed fsync local snapshot и idempotent orphan adoption | гарантия rename/share modes зависит от конкретного SMB server family; каждый supported family требует live contract qualification (`OPS-8`) |
| **T/I/E** — producer либо лишняя identity входит в private managed-import namespace | неверная share/filesystem ACL открывает processing/terminal/quarantine | `SEC-DATA-3` (Designed): operator-provisioned fixed namespace, distinct producer/service identities, service positive capability gate и operator-run producer-denial test | приложение не может полноценно аудировать effective ACL другого principal; до ADR-0025 H2 контроль не enforced (`OPS-3`) |
| **I/D/R** — remote terminal/quarantine evidence остаётся бессрочно или теряет cleanup authority | existing retention удаляет local/ledger evidence без source-side purge | `SEC-DATA-3` (Designed): exact regular-file remote purge первым, idempotent absence, contradiction fail-closed, terminal ledger row последним; replay source-detached | до ADR-0025 H1/H3 remote copies не bounded (`OPS-9`) |
