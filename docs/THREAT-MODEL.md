...
| **T/I** — SQL injection из IOC/metadata | недоверенное значение меняет структуру durable query | `SEC-INP-3`: business values bind'ятся через `JdbcClient`/`PreparedStatement`; dynamic identifiers проходят allow-list validation + quoting | контроль частично ручной: новые dynamic SQL entry points требуют review |
