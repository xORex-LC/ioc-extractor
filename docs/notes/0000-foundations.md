...
Стартовые решения по стилю, стеку и набору артефактов, принятые до появления
журнала `notes/`. Зафиксированы здесь для полноты истории.
...
| 6 | Выходные артефакты | **Два**: сетевые маски + файловые хэши (раздельные схемы/id/нормализация) | Соответствует эталонам; маршрутизация по `IndicatorType` | Один общий артефакт |
...
- Эталоны/историю развития последующих решений см. в нотах 0001–0005.

## Открытые вопросы

Нет; последующие уточнения вынесены в профильные ноты.
