...
| **PatternEngine** | regex-движок | `PatternEngine` | regex → matches | RE2J / JDK | adapter | `platform-regex` |
...
| **LookupRepository** | лукап/дедуп-стор | `LookupRepository` | Indicator → bool, maxId | CSV | adapter | `adapter-lookup-csv` |
| **Diagnostics** | сбор/рендер диагностики, каталог | `DiagnosticSink`, `DiagnosticRenderer`, `MessageCatalog` | Diagnostic → msg/log/report | — | cross-cutting | `platform-diagnostics` |
...
| **Watch ingest** *(future)* | приём источников: детект/клейм, вызов use case | `IngestSourceUseCase` (in); `SourceFeed` — adapter-local | dir → SourceUnit | Spring Integration | adapter (in) | `adapter-ingest` |
| **IngestionLedger** *(future)* | журнал обработанного (идемпотентность) | `IngestionLedger` | unit → status | file/SQLite | adapter | `adapter-ingest` |
| **Aggregator** *(future)* | свод партиций → канон. артефакт | `Aggregator` | partitions → artifact | — | application | `ioc-application` |
...
```
  ...
  future (daemon): watch ─IngestSourceUseCase─▶ [тот же конвейер] ─▶ партиции ─Aggregator─▶ артефакт ─PartitionReaper⟳
```
