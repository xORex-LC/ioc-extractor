...
| `SEC-CI-6` | Внешний Codecov coverage reporting | Configured; operational proof pending | отдельный least-privilege job получает только `contents: read` + OIDC `id-token: write`, скачивает уже проверенный CI artifact и передаёт full-SHA Action с pinned CLI только aggregate JaCoCo XML; auto-search/plugins/telemetry выключены, PR comment выключен, annotations и три informational status включены; status не required, outage не блокирует PR/release |
