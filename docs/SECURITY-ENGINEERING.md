...
| `SEC-SCA-2` | Dependabot dependency/malware alerts, security updates и weekly version updates для Maven/Actions | Configured | Alerts #105–109 закрыты после merge maintenance patch в `main` 2026-09-09; [релиз 0.2.1](https://github.com/xORex-LC/ioc-extractor/releases/tag/v0.2.1). Версии перенесены в 0.3.0; [dependabot.yml](../.github/dependabot.yml) |
| `SEC-SCA-3` | OWASP Dependency-Check aggregate по effective reactor graph | Configured | [Запуск 34083754147](https://github.com/xORex-LC/ioc-extractor/actions/runs/34083754147) от 2026-09-07 обновил NVD и сохранил HTML/JSON; scan завершился ошибкой: 35 findings (10 Critical, 9 High, 14 Medium, 2 Low). [Scan 34349325936](https://github.com/xORex-LC/ioc-extractor/actions/runs/34349325936) на release commit `6138464d` прошёл: 128 dependencies, 0 неподавленных findings и две действующие JBIG2 suppressions. Перенос в 0.3.0 требует отдельного exact-HEAD scan |
...
| `SEC-CI-2` | NVD secret isolation и read-only security job | Configured | job явно привязан к Environment `SECURITY CHECKS`, `NVD_API_KEY` доступен только explicit update step; следующий offline scan не получает secret, token permissions ограничены `contents: read`; update/cache/report steps успешно выполнены в [запуске 34083754147](https://github.com/xORex-LC/ioc-extractor/actions/runs/34083754147); Environment требует reviewer approval (проверено 2026-09-08) |
...
| `SEC-REL-2` | Exact-candidate dependency scan до tag | Configured | manual `Dependency Security` run является release gate; 0.2.1 квалифицирован [run 34349325936](https://github.com/xORex-LC/ioc-extractor/actions/runs/34349325936). Этот отчёт не квалифицирует изменённый reactor 0.3.0; ему требуется собственный exact-commit scan |
