...
**Post-v1 уточнение модели: detection и execution должны быть разделены.** Практический
прогон SMB-поставки показал, что v1 корректно доставляет данные, но `RemoteFetchService` и
`ArtifactPublishService` частично совмещают две ответственности: обнаружение работы и её
исполнение. В частности, fetch делает remote `list` внутри use-case; этот `list` одновременно
является проверкой изменения remote source и началом fetch. При протухшей SMB idle-сессии
это может давать transient `DiskShare has already been closed` и краткий `DOWN` в sync health,
хотя следующий tick переоткрывает transport и данные продолжают двигаться.

Целевая модель для следующего hardening-прохода — **state-driven sync с reconcile safety net**:

```text
RemoteSourceMonitor
  └─ строит remote snapshot/fingerprint и emits changed-work
     └─ RemoteFetchService скачивает конкретные новые identities

PublishWorkDetector
  └─ сверяет completed slices × configured targets × publish_ledger
     └─ ArtifactPublishService публикует конкретные pending/failed pairs
```

Fetch не должен владеть собственным рабочим таймером; таймер/notification belongs to
remote-state detector. Publish может иметь собственную cadence/window-политику, но она должна
жить на уровне detector/reconciler, а не внутри delivery executor. Полный отказ от reconcile
нежелателен: durable ledgers остаются источником истины после рестартов, потерянных событий и
crash windows. События/WatchService/transport-native notifications — оптимизация latency,
а не единственная гарантия correctness.
...
## Группировка срезов для реализации
