...
- **Идемпотентность / forward-recovery:** срез immutable (timestamp-имя); падение до
  ...
  частичное состояние, маркер один раз в конце. Стыкуется с фазой `PUBLISHED` export-саги
  (решение 6).
...
- **Трансформации — адаптерные seam'ы, не форма порта.** Раз publish — толстый intent
  (реш. 14), байт-конвейер живёт в адаптере: сжатие (`GZIP` — бьёт по transfer-amplification
  `complete` из 0012), клиент-сайд шифрование (поверх SMB3-транзита, для недоверенной шары),
  стриминг-хэш (`DigestInputStream` — checksum за один проход, под верификацию реш. 14).
  Будущие опции `ioc.sync.publish.compress`/`encrypt` — внутри адаптера, порт не меняется.
...
- **Локальные срезы** (`var/export/...`, наши) — **reaper ING-1** (max-age/max-count);
  `complete`-срезы самодостаточны → хранить минимум (0012 Q5).
...
- **Ретеншен** (реш. 18): локальные срезы — reaper ING-1; приёмник — за получателем по
  умолчанию (мы только пишем, least-privilege), opt-in `ioc.sync.publish.retention` где владеем
  приёмником.
