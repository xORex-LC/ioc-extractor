...
5. ✅ **Решено — reaper (ING-1) для локальных срезов; приёмник — концерн доставки.**
   Накопление immutable-срезов в `var/export/...` чистит существующий reaper (max-age/
   max-count); `complete`-срезы самодостаточны → хранить минимум (вплоть до 1 + `_SUCCESS`).
   Ретеншен **на приёмнике** — за доставкой: по умолчанию за получателем, opt-in publish-side
   в [0011](0011-remote-sync.md) реш. 18 (не реапим чужую шару из 0012).
