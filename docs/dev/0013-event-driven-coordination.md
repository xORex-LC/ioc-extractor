...
- **Transactional Outbox** (Richardson, microservices.io) — именованный ответ на durable
  ...
  **«БД + брокер»**; у нас брокера нет → классический outbox YAGNI (реш. 11), но под-проблему
  «атомарность state-change + dispatch при краше» закрывает Registry ниже.
...
> плодить **два** механизма поллинга, выбор «монитор на SI vs на `PeriodicDaemonCycle`» оставлен
> открытым (откр. вопрос 9): SI оправдан, только если даёт сверх простого поллера то, что
> `PeriodicDaemonCycle` не закрывает. И терминологически: монитор **двигает метаданные и рождает
...
ING-4) множество completed-срезов и их `manifestSha256` durable в БД — поэтому reconcile-backstop
может быть **DB-driven** (запрос pending-пар по `publish_ledger`, O(pending)), а не filesystem-rehash
(O(срезы×файлы)). Иначе «дорогая идемпотентная работа вхолостую» из OPS-4 убирается с fast-path, но
**остаётся на медленном пути**, лишь реже (откр. вопрос 6).
...
- **Shutdown / rejection policy (часть delivery-semantics, не мелочь):** очередь по ключу —
  **bounded** (backpressure, а не безграничный рост); при остановке демона — **graceful drain** с
  ...
  (R32/B4):** если ключ хронически переполнен (медленный SMB), новые enqueue + reconcile-re-emit
  упёрлись бы в одну полную очередь (риск livelock) — поэтому при red-health очереди (реш. 10)
  издатель/монитор **перестаёт enqueue'ить fast-path и отдаёт ключ целиком reconcile'у**, а не долбит
  rejection'ами (точная граница — откр. вопрос 7).
...
- **E4 — fetch detection/execution split.** `RemoteSourceMonitor` (SI poll + include/exclude +
  ...
  монитор на SI vs `PeriodicDaemonCycle` — откр. вопрос 9 (A3).
...
6. **DB-driven reconcile vs filesystem-rehash** — после ING-4 reconcile-backstop может читать
   `publish_ledger`/манифесты из SQLite вместо пере-хеша ФС в `listCompleted`. Открыто: вводим ли
   `CompletedSliceCatalog.find` + DB-backed reconcile в E3/E5, или оставляем FS-scan редким
   (реш. 7/R30, B2).
7. **Admission-политика насыщенного ключа** — точная граница, на которой fast-path перестаёт
   enqueue'ить и отдаёт ключ reconcile'у (по red-health очереди), чтобы bounded-queue + reconcile не
   зацикливались на медленной цели (реш. 9/R32, B4).
...
9. **Монитор на SI или на `PeriodicDaemonCycle`** — не заводить два механизма поллинга; решить, что
   SI даёт сверх общего цикла (реш. 4/12, A3).
