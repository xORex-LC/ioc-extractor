...
| OPS-4 | **State-driven sync / event coordination hardening.** Базовая часть закрыта ADR 0013: `RemoteSourceMonitor` отделён от `RemoteFetchService`, publish fast-path идёт через `SliceCompleted`, periodic reconcile остаётся correctness backstop, fetch/publish work сериализуется in-memory keyed executor по endpoint, actuator показывает queue/degradation state. Открытые seam'ы: transport-native notification (`CHANGE_NOTIFY`/SI push), durable outbox/broker adapter при межпроцессной доставке, полноценная high/low-water hysteresis policy вместо текущего bounded admission + shed-to-reconcile. | seam | M | dev/0013, docs/sync.md |
