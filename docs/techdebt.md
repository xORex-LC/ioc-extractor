...
| EXP-6 | **Publish-ledger retention guard** — standalone guard разрешает локальный slice delete; при реализации 0011 его заменяет `PublishLedgerSliceRetentionGuard`, где missing/pending target pair pin-ит срез. Port, late lifecycle phase и check-before-delete уже реализованы. | seam | M | dev/0011, dev/0012 F2 |
