...
| ING-4 | **Durability ledger + сторадж** — частично реализован служебный JDBC storage: `ioc.ingestion.ledger.type: file \| jdbc`, service SQLite datasource, `user_version`-миграции, JDBC `IngestionLedger`, legacy-import `.properties` → `legacy_imports`, DB health (`user_version`/PRAGMA/`quick_check`). Осталось: business dataframe truth, CSV как проекция, run-ledger/saga и retention через запросы к стору. Сюда же сводятся два retention-вопроса, которым нужен запрос к стору, а не обход ФС: **(а)** развязка retention↔агрегации (реапить только заведомо агрегированные партиции, а не по голому возрасту/количеству); **(б)** per-group count-retention (счёт «N новейших на артефакт/группу», а не по плоскому пулу листьев). | частично | M | dev/0001, review, worknote/storage-layer |
