...
| OPS-4 | **State-driven sync вместо рабочих таймеров fetch/publish.** 0011 v1 работает как polling/reconcile loop, но смешивает detection и execution: fetch сам делает remote `list`, publish сам ищет локальный worklist. Цель hardening: `RemoteSourceMonitor`/snapshot detector → `RemoteFetchService` для конкретных identities; `PublishWorkDetector` → `ArtifactPublishService` для pending/failed pairs. Reconcile оставить как safety net для restart/crash-window, но idle publish не должен трогать SMB, а transient stale SMB share (`DiskShare has already been closed`) не должен кратко валить health. | seam | M | эксплуатация 0011, docs/sync.md |
