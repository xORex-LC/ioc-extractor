...
| OBS-D3 | **ECS-типы строками.** Закрытие спроектировано в ADR-0018: ambient string correlation остаётся в MDC, event-local fields переходят на SLF4J 2 typed key/value pairs; `LogField` становится исполнимой JSON type schema; Boot 3.4 built-in ECS formatter заменяет Elastic encoder. До реализации duration/counts/boolean в daemon JSON остаются строками. | открыт | M | ADR/0018 |
