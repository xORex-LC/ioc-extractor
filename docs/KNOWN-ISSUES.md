...
| ING-10 | **Гонка startup recovery ⊥ ingestion poller.** `IngestionRecoveryRunner` — `ApplicationRunner` (после refresh), а SI-poller — `SmartLifecycle` с `autoStartup=true` (стартует во время refresh) → файл на старте обрабатывается двумя прогонами: дубль-экстракция, задвоенные diagnostics, застрявший `PROJECTION_COMPLETED`-run, ложный `INGEST.DEAD_LETTER_FAILED`. `concurrency: 1` не помогает (разные lifecycle-ветки). C2 SpotBugs review дополнительно обнаружил в `JdbcIngestionLedger` неиспользуемые `TransactionTemplate`/`inTransaction` и blind find→update в terminal transitions: fix должен включать expected-state/CAS semantics и concurrent adapter test, а не просто transaction wrapper. Данные целы (row_key keep-first). Фикс: `autoStartup=false` + старт flow по завершении recovery (lifecycle phase/gate) + per-source-key сериализация + атомарные ledger transitions. Митигация до фикса: planned restart выполнять только в idle maintenance window после остановки intake и проверки пустых `inbox`/`processing`; это снижает exposure, но не заменяет lifecycle barrier (см. operator runbook). **План: 0.3.0.** | открыт | M | стенд-тест 2026-07-14 (pre-retarget RC 0.1.1), BUILD-SPOTBUGS-04/C2 review 2026-08-01 |
...
1. **0.3.0 — ingest lifecycle hardening:** `ING-10`, `ING-11` (с мини-дизайном
