...
| ING-10 | **Startup recovery отделён от ingestion intake.** Flow имеет `autoStartup=false`; единый coordinator последовательно восстанавливает run/source ledgers и только затем запускает poller. Ingest/recovery/reject сериализуются по `SourceKey`, recovery перечитывает durable state после admission, а file/JDBC ledgers используют монотонные expected-state transitions. Health показывает lifecycle и aggregate contention без source identities; `concurrency` fail-closed закреплён на `1`. Latch-tests, общий concurrent adapter TCK, restart regression и watched-inbox E2E закрывают исходную гонку и `SB04-116`. Гарантия process-local; multi-daemon требует lease/fencing. | закрыт | M | checkpoints `f4f011e`, `c3a03e2`, `a44d10f`, `7ce5f8f`; [ING-10 worknote](worknote/0.3.0/ing-10-ingestion-lifecycle-hardening.md), 2026-08-02 |
