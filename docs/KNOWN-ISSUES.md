...
| OPS-7 | **Managed-import SMB server-family breadth.** Live ownership/materialization/disposition/notification contract квалифицирован на Samba. Перед заявлением поддержки конкретного Windows Server/NAS family нужно прогнать тот же adapter contract: server-side rename/share modes, stable file ID, disconnect recovery и `CHANGE_NOTIFY`. Отсутствие такого стенда — qualification skip, а не pass; polling заменяет только потерянные notifications, но не ownership checks. | qualification | M | DATA-IMPORT-01 P8/P9 |
