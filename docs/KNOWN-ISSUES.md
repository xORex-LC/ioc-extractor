...
| ING-13 | **Failed claim превращает файл в вечный poison.** Полумера 0.1.1 реализована: rejection возвращает `REJECTED/ALREADY_REJECTED`, повторный poll durable `FAILED` завершается без повторного generic/typed лога. Полный дефект остаётся: `markFailed` синтезирует `Path.of("unknown")`, файл остаётся в inbox и не получает явной физической судьбы. Нужен pre-claim dead-letter/quarantine (с реальными path/detectedAt и идемпотентным terminal-фильтром); после устранения причины текущий runbook — удалить ledger-запись и повторно подать файл. **Полный фикс — 0.1.2.** | открыт | M | стенд-тест 2026-07-14 (RC 0.1.1), полумера 2026-07-15 |
...
| OBS-D3 | **ECS-типы строками.** Закрыто ADR-0018: ambient string correlation остаётся в MDC, event-local fields идут через SLF4J 2 typed key/value pairs; `LogField` — исполнимая JSON type schema; Boot 3.4 `StructuredLogEncoder` заменил Elastic encoder. JSON type matrix, collision и async hand-off закреплены тестами. | закрыт | M | ADR/0018; `a8867f9`, `e4e0ea5`, `FEATURE: emit typed ECS structured log fields` |
