...
Архитектурное направление остаётся корректным: file-ingest как driving-adapter,
существующий `ExtractIocsUseCase` как ядро обработки, идемпотентность через
durable ledger, выход через партиции. Это соответствует текущей модели
Pipes-and-Filters/Envelope и не требует тащить Spring Integration внутрь
application/domain.

Что нужно уточнить перед реализацией:

- **Режим приложения** не должен управляться только `ioc.observability.mode`.
  Нужен runtime mode (`ioc.runtime.mode: oneshot | daemon`), а observability
  читает его как поле логов.
- Текущий `IocExtractorApplication` вызывает `System.exit(...)` безусловно.
  Для daemon это недопустимо: в `daemon` процесс должен жить, пока жив Spring
  context/SI-flow.
- Текущий `CliRunner` всегда активен. Для daemon он должен быть выключен через
  condition/profile, иначе приложение будет печатать CLI usage и завершаться
  не тем жизненным циклом.
- Текущий `IocSink.write(List<Indicator>)` не получает контекст источника.
  Для partition output используем **отдельный partition wrapper в adapter layer**:
  application вызывает тот же `ExtractIocsUseCase`, а bootstrap в daemon-режиме
  подставляет sink wrapper, который строит путь партиции из контекста источника
  (`sourceId`/content hash) и делегирует CSV-запись существующему mapper/sink.
- Этап 10 не должен пытаться сразу решать глобальные стабильные id. Это зона
  этапа 11: single-writer aggregator + `dedupKey -> id`.
...
инвариант stage 10, а не оптимизация.
...
частичных файлов при падении не возникает. Для stage 10 достаточно довести юнит
до `SOURCE_ARCHIVED`; `AGGREGATED` появляется на этапе 11.
...
- Категоризация и трансляция ошибок — через diagnostics: stage 10 должен начать
  закрывать долг D1 из [roadmap.md](roadmap.md), минимум для `SOURCE.READ_FAILED`,
  `INGEST.CLAIM_FAILED`, `INGEST.LEDGER_WRITE_FAILED`, `SINK.WRITE_FAILED`.
...
- **Spring Integration file support** — основной implementation reference для
  stage 10: [file inbound adapter / reading files](https://docs.spring.io/spring-integration/reference/file/reading.html),
