...
Этапы 0–9 реализованы: проект переведён в Maven-реактор с модулями
`platform`, `core`, `adapters` и `bootstrap`; ядро, диагностика, pipeline,
observability и текущие адаптеры собираются и проверяются через reactor build.
Документы по архитектуре и границам — живые: расширяются по мере перехода к
daemon-ingestion и агрегации.
