...
|---|---|---|---|---|---|---|
| **Refanger** | деобфускация defanged-IOC | `Refanger` | text → `RefangOutcome` (text + applied rule counts) | refang-rules (config) | domain | `ioc-domain` |
| **IndicatorExtractor** | детекция индикаторов | `IndicatorExtractor`, `PatternEngine`(SPI) | text → `ExtractionOutcome` (kept/dropped decisions) | PatternEngine, patterns | domain | `ioc-domain` |
| **PatternEngine** | regex-движок | `PatternEngine` | regex → matches | RE2J / JDK | adapter | `adapter-regex-re2j` |
| **SourceAttributor** | атрибуция `source` по маркерам | `SourceAttributor` | text+raw → `AttributionOutcome` | markers (config) | domain | `ioc-domain` |
...
| **MatchClassifier** | коды масок (4-вар., rule-based) | `MatchPolicy` | Indicator → `ClassificationDecision` (features + rule/predicates + MaskMatch) | classify-rules | domain | `ioc-domain` |
...
| **Diagnostics** | impact/catalog, bounded processing outcome, policy и delivery ports | `DiagnosticSink`, `DiagnosticRenderer`, `FailurePolicy` | Diagnostic batches → policy/summary/delivery | — | cross-cutting | `platform-diagnostics` |
...
| **Diagnostics logging bridge** | resilient/redacted запись diagnostics в общий лог-поток | `LoggingDiagnosticSink`, `ResilientDiagnosticSink` | Diagnostic → LogEvent | diagnostics + observability | cross-cutting | `platform-diagnostics-logging` |
| **Pipeline decision tracing** | gated structured TRACE уже вычисленных per-item outcomes | `PipelineDecisionTracer` | decision → `pipeline_item_decision` | application outcomes, logger gate | application + bootstrap | `ioc-application`, `ioc-app` |
...
| **RemoteFetchService** | fetch execution для detected identities; explicit CLI fetch также владеет retried listing; atomic landing через скрытый staging в local inbox | `RemoteFetchUseCase`; `FileTransport`, `RemoteFetchLedger` | remote selection/detected objects → fetched/skipped/failed | transport, fetch ledger, inbox filesystem | application | `ioc-application` |
| **SyncDiagnosticReporter** | final transport-neutral `RemoteErrorKind` → `SYNC.*` после retry/durable failure transition | `DiagnosticSink` | final remote failure → Diagnostic | sync error axis | application | `ioc-application` |
