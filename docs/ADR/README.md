...
| `0020-canonical-record-expiration-lifecycle.md` | Record validity принадлежит canonical artifact lifecycle; internal `valid_until`, exact active-read predicate, bounded expiration/history reconciliation, monotonic IDs, explicit one-way legacy activation, clock safety и duplicate-receipt fallback образуют один согласованный data-quality contract. Existing public schemas остаются неизменными. **Принято 2026-08-16; P1 application contracts реализованы, storage/runtime ещё не активны.** |
