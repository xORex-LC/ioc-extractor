...
| `0021-stable-reusable-export-slots.md` | Внешний artifact `id` отделяется от canonical/lifecycle identity и трактуется как stable sparse reusable export slot: surviving active rows не перенумеровываются, expired slots освобождаются при следующем eligible export, новые rows занимают минимальные holes. Узко supersede'ит ADR-0020 §4. **Proposed 2026-08-19; implementation/evidence pending.** |
