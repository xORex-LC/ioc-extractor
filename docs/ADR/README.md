...
| `0016-config-preflight-strict-binding.md` | Надёжность конфигурации (блок CFG): единый startup preflight для `ioc.*` до runtime graph; semantic collect-all через `configurationPropertiesValidator`, строгий binding unknown/deprecated keys, ссылочная целостность config→config (identity↔sink, key-columns⊆columns, дубликаты, id policy); compact-конструкторы — только нормализация/дефолты; tombstones удаляются из модели, миграционные подсказки — в кастомном `FailureAnalyzer` (без связки с diagnostics); закрытые словари → enum/value types, ранние consumers и `IocProperties` используют одну грамматику, `id.start` получает единый parser/value contract. **Принято, не реализовано.** |
