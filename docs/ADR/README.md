...
| `0018-typed-ecs-structured-logging.md` | Типизированный ECS logging contract (OBS-D3): ambient string correlation остаётся в MDC, event-local duration/counts/boolean идут через SLF4J 2 key/value pairs; `LogField` получает исполнимый JSON scalar type и generated catalog, arbitrary keys/numeric MDC запрещаются. Spring Boot 3.4.13 владеет ECS formatter-ом в bootstrap; Elastic encoder удалён. Wire-format migration, collision semantics, async/concurrency и будущий generated Elasticsearch component-template seam описаны явно. **Принято и реализовано 2026-07-16 (`da9ff3f`, `a8867f9`, `e4e0ea5`, feature-коммит typed transport).** |
