...
| `0018-typed-ecs-structured-logging.md` | Типизированный ECS logging contract (OBS-D3): ambient string correlation остаётся в MDC, event-local duration/counts/boolean идут через SLF4J 2 key/value pairs; `LogField` получает исполнимый JSON scalar type и generated catalog, arbitrary keys/numeric MDC запрещаются. Spring Boot обновляется до 3.4.13 и владеет ECS formatter-ом в bootstrap; Elastic encoder удаляется. Wire-format migration, collision semantics, async/concurrency и будущий generated Elasticsearch component-template seam описаны явно. **Принято 2026-07-16, не реализовано.** |
