...
| `R030-BUILD` | `in-progress` | [build-quality ledger](evidence/build-quality-ledger.md) | Surefire/Failsafe separation and CI test/coverage retention are verified; finish coverage ratchets, Codecov evidence and required-status qualification with `R030-TEST` |
| `R030-TEST` global controls | `in-progress` | [test-quality ledger](evidence/test-quality-ledger.md) | `TEST-LIFECYCLE-01` verified; proceed with `TEST-COVERAGE-02` on the stable post-split universe |
