...
| `R030-BUILD` | `in-progress` | [build-quality ledger](evidence/build-quality-ledger.md) | Resume `BUILD-SPOTBUGS-04/C3` from the verified `ING-10`/`IR-03` predecessor and reconciled 119-finding report |
...
| `BUILD-SPOTBUGS-04` | `R030-BUILD` | Original 118-finding inventory plus verified post-ING-10/follow-up deltas and both audit follow-ups: 119 current raw findings in the same 19-module production-bytecode scope | `C0..C2` completed; original inventory is now classified as 55 false positives, 55 accepted legacy, 3 fix-now and 5 resolved findings. Verified `IR-03` removed `SB04-116`; typed/literal health PRAGMA removed `SB04-004..005`; exception-preserving guard release removed `I4-SB-01`. Three remaining ING-10 findings and one observability-follow-up exception-flow finding are individually classified false positives and unsuppressed for current-report reconciliation in `C3` | AI agent | `in-progress` | [execution worknote](build-spotbugs-04-worknote.md) |
