...
| `BUILD-PMD-06` | `R030-BUILD` | Same fail-closed 25-project disposition and 19 production `src/main/java` roots as CPD; root, TCK, tests, generated/vendor sources and build-only POMs excluded | P3 adopted a reduced 22-rule report-only policy with calibrated `CognitiveComplexity=16` and `ExcessiveParameterList.minimum=13`; 3 actionable and 3 intentionally ignored-parameter occurrences removed. A separate executable 3-rule watchlist preserves ownership/size signal without regular-CI adoption; 7 rules dropped and 2 PMD-8 rules replaced. Regular separate CI job blocks analyzer/scope/ruleset/report failures and uploads XML/HTML, while 6 policy findings remain visible advisory debt/false-positive evidence and no suppression/baseline is allowed. Harness: 7 happy / 48 negative scenarios | AI agent | `verified` (`P0/P1/P2/P3` completed) | [PMD adoption worknote](build-pmd-06-worknote.md) |
