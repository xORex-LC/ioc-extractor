...
| `R030-TEST` global controls | `in-progress` | [test-quality ledger](evidence/test-quality-ledger.md) | `TEST-LIFECYCLE-01` through `TEST-PILOTS-06` are verified; `TEST-CODECOV-07` has operational upload evidence and an explicit external-status disposition; continue with accepted consumer gaps while `TEST-PUBLICATION-08` waits on `R030-LIB` |
