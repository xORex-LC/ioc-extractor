...
| `R030-BUILD` | `in-progress` | [build-quality ledger](evidence/build-quality-ledger.md) | Audit remaining goal-level DoD and branch-protection evidence; SpotBugs blocking adoption is closed |
...
| `BUILD-SPOTBUGS-04` | `R030-BUILD` | C4 closure baseline: 77 reviewed findings in the same 19-module production-bytecode scope; one inherited filter had 71 exact selectors for 59 false positives + 18 policy-noise findings and no accepted legacy | Exception boundaries, 11 small local findings and all 44 mutable aliases were hardened with focused regressions. C4 also replaced compiler-generated lambda names with a stable exact class/field selector. Final clean + immediate repeat both passed 24/24 with 836 tests, 2 external SMB skips, 19 module report pairs + aggregate, 0 visible findings and 0 analyzer errors/missing classes. Later concurrency and Tika path follow-ups removed `SB04-016` and `SB04-029..030`, leaving 74 accepted identities / 68 selectors; Tika has focused test and SpotBugs evidence, while the full reactor was not rerun for that last follow-up | AI agent | `verified` | [execution worknote](build-spotbugs-04-worknote.md) |
| `BUILD-SPOTBUGS-05` | `R030-BUILD` | Same 19 production JAR modules; 74 exact accepted identities (56 false positives + 18 policy noise), 68 generated narrow selectors, test bytecode still excluded | One raw analysis per module; root validates the structured baseline and materializes the filter; late gate blocks new/stale/moved/metadata drift and analyzer/report/aggregate failures. Latest full 24/24 run remains the 76-raw snapshot; subsequent focused Tika verification removed two findings and passed with 0 source-adapter findings. A full reactor refresh was explicitly skipped | AI agent | `verified` | [blocking-ratchet worknote](build-spotbugs-05-worknote.md) |
