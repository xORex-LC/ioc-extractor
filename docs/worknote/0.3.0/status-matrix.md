...
| Full PMD ruleset beyond CPD | `R030-BUILD` | `deferred` | Unused-code, complexity, performance/object-creation and error-prone rules need their own signal/noise decision; they are not implied by CPD. Prerequisite `BUILD-SPOTBUGS-04` is satisfied, but no adoption decision has been made | Explicit scope/priority decision with risk, desired signal and bounded work item; this evaluation is not required to close `R030-BUILD` |
