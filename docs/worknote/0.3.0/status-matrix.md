...
| `BUILD-SPOTBUGS-04` | `R030-BUILD` | C4 closure baseline: 77 reviewed findings in the same 19-module production-bytecode scope; one inherited filter had 71 exact selectors for 59 false positives + 18 policy-noise findings and no accepted legacy | Exception boundaries, 11 small local findings and all 44 mutable aliases were hardened with focused regressions. C4 also replaced compiler-generated lambda names with a stable exact class/field selector. Final clean + immediate repeat both passed 24/24 with 836 tests, 2 external SMB skips, 19 module report pairs + aggregate, 0 visible findings and 0 analyzer errors/missing classes. A later concurrency follow-up removed `SB04-016`, leaving 76 accepted identities / 70 selectors | AI agent | `verified` | [execution worknote](build-spotbugs-04-worknote.md) |
| `BUILD-SPOTBUGS-05` | `R030-BUILD` | Same 19 production JAR modules; 76 exact accepted identities (58 false positives + 18 policy noise), 70 generated narrow selectors, test bytecode still excluded | One raw analysis per module; root validates the structured baseline and materializes the filter; late gate blocks new/stale/moved/metadata drift and analyzer/report/aggregate failures. Latest 24/24 run: 837 tests, 2 external skips, 19 report pairs + aggregates, 76 raw / 0 visible, analyzer errors/missing classes 0/0 | AI agent | `verified` | [blocking-ratchet worknote](build-spotbugs-05-worknote.md) |
