...
|---|---|---|---|---|---|---|
| `DATA-AGGREGATE-01` | `R030-DATA` | Новый самостоятельный IOC aggregate artifact; обновление name при совпадении остальных public fields | Discovery: IPv4, домены, URL и хеши, пустой старт, managed import новой схемы и конфигурируемая artifact policy подтверждены; приоритет по регистрации, последнее непустое вхождение внутри документа и сохранение name при пустом значении согласованы; технический проект подготовлен, включая persistence/recovery и оценку библиотек; вопросы export/import/storage-mode отмечены явно; implementation не начата. Ранее verified TTL/import scope не квалифицирует эту фичу | AI agent + user review | `analyzing` | [рабочий комплект](ioc-aggregate/README.md), [discovery](ioc-aggregate/discovery.md), [draft release contract](ioc-aggregate/release-contract.md), [technical design](ioc-aggregate/technical-design.md) |
