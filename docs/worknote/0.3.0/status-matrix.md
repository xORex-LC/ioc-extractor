...
| `R030-DATA` | `in-progress` | [TTL evidence](data-ttl-01/evidence.md), [P6 load profile](data-ttl-01/evidence/p6-load-profile.md), [release contract](data-ttl-01/release-contract.md) | Run privileged disposable-host systemd fresh/upgrade/rollback stand, then final candidate gates |
...
|---|---|---|---|---|---|---|
| `DATA-TTL-01` | `R030-DATA` | Canonical record expiry across application contracts, SQLite truth/history, active reads, IDs, projection recovery, ingestion duplicates, startup/config/health and packaged activation | P0–P5 complete; P6 fresh preset/docs/rootless 100k profile complete; privileged packaged rollout and final RC gates pending | AI agent + user review | `in-progress` (`P6`) | [evidence](data-ttl-01/evidence.md), [P6 load profile](data-ttl-01/evidence/p6-load-profile.md), [implementation plan](data-ttl-01/implementation-plan.md), [ADR-0020](../../ADR/0020-canonical-record-expiration-lifecycle.md) |
