...
| `R030-BUILD` | `in-progress` | [build-quality ledger](evidence/build-quality-ledger.md) | Required project-owned statuses are active on `main` and `release-0.3.0`; one pull request must now pass the five release-branch checks under that policy |
