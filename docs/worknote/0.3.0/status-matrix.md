...
| `R030-BUILD` | `in-progress` | [build-quality ledger](evidence/build-quality-ledger.md) | Lifecycle separation, CI retention, coverage universe/integrity, ratchets and fixed floors are verified; finish Codecov evidence and required-status qualification with `R030-TEST` |
| `R030-TEST` global controls | `in-progress` | [test-quality ledger](evidence/test-quality-ledger.md) | `TEST-LIFECYCLE-01`, `TEST-COVERAGE-02`, `TEST-REGEX-03`, `TEST-WAITS-04` and target-scoped `TEST-EXTERNAL-05` are verified; continue with `TEST-PILOTS-06` |
