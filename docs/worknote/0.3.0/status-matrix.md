...
| `R030-BUILD` | `in-progress` | [build-quality ledger](evidence/build-quality-ledger.md) | `BUILD-PMD-06/P2`: semantically triage the 92-finding PMD baseline and measure overlap/threshold sensitivity; SpotBugs blocking adoption remains closed |
...
| `BUILD-PMD-06` | `R030-BUILD` | Bounded source-analysis evaluation over the same 19 production `src/main/java` roots as CPD; fail-closed 25-project disposition excludes root, TCK, tests, generated/vendor sources and other build-only POMs | P0 selected 34 exact rules. P1 proved Maven PMD Plugin `3.28.0` + PMD `7.26.0` on JDK 21, deterministic XML/HTML and an opt-in selected-reactor command: 92 raw findings / 48 files (A 6, B 25, C 37, D 24), no PMD suppressions/baseline/code remediation and no regular CI adoption | AI agent | `in-progress` (`P0/P1` completed; `P2` pending) | [PMD evaluation worknote](build-pmd-06-worknote.md) |
