...
| `BUILD-SPOTBUGS-04` | `R030-BUILD` | C3 raw snapshot: 114 reviewed findings in the same 19-module production-bytecode scope; one inherited baseline has 109 exact selectors for 55 accepted legacy + 41 false positives + 18 policy-noise findings | `C0..C3` completed. `IR-01`, `IR-02` and predecessor `IR-03` are fixed with regressions; exception-flow follow-up hardened five primary-failure boundaries without changing the 18 exact `THROWS_*` selectors. All 19 module report pairs plus aggregate contain 0 visible findings with analyzer errors/missing classes 0/0. Findings remain report-only and unmatched new signal remains visible. Next: remediate small legacy groups, then mutable aliases and C4 repeat | AI agent | `in-progress` | [execution worknote](build-spotbugs-04-worknote.md) |
