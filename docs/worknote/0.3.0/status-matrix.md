...
| `R030-BUILD` | `in-progress` | [build-quality ledger](evidence/build-quality-ledger.md) | Lifecycle separation, CI retention, coverage universe/integrity and no-regression ratchets are verified; finish fixed floors, Codecov evidence and required-status qualification with `R030-TEST` |
| `R030-TEST` global controls | `in-progress` | [test-quality ledger](evidence/test-quality-ledger.md) | `TEST-COVERAGE-02` universe, missing-report integrity and ratchets are implemented; remediate aggregate/domain/application gaps before enabling fixed floors |
