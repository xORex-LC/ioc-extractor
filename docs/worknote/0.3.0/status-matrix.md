...
| `R030-BUILD` | `in-progress` | [build-quality ledger](evidence/build-quality-ledger.md) | Run `BUILD-SPOTBUGS-04/C4`: clean reactor plus immediate repeat must reproduce the C3 accepted signal and complete report set |
...
| `BUILD-SPOTBUGS-04` | `R030-BUILD` | Current remediation baseline: 103 reviewed findings in the same 19-module production-bytecode scope; one inherited filter has 98 exact selectors for 44 accepted legacy + 41 false positives + 18 policy-noise findings | `C0..C3` completed. Exception-flow follow-up hardened five primary-failure boundaries; all 11 small local legacy findings are resolved with focused regressions and their selectors removed. Final 19 module reports plus aggregate contain 0 visible findings, analyzer errors and missing classes. Findings remain report-only and unmatched new signal remains visible. Next: remediate 44 mutable aliases, then C4 clean + immediate repeat | AI agent | `in-progress` | [execution worknote](build-spotbugs-04-worknote.md) |
