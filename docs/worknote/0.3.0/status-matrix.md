...
| `BUILD-PMD-06` | `R030-BUILD` | Same fail-closed 25-project disposition and 19 production `src/main/java` roots as CPD; root, TCK, tests, generated/vendor sources and build-only POMs excluded | P3 adopted a reduced 22-rule report-only policy with calibrated `CognitiveComplexity=16` and `ExcessiveParameterList.minimum=13`; a separate executable 3-rule watchlist preserves ownership/size signal without regular-CI adoption. Post-import requalification removed bounded charset, unused-parameter, empty-catch and exception-precedence defects; 21 policy findings in 16 files and 29 watchlist findings in 19 files remain visible as reviewed advisory debt/false positives, with no suppression/baseline. Harness: 7 happy / 48 negative scenarios | AI agent | `verified` (`P0/P1/P2/P3` completed; requalified 2026-08-31) | [PMD adoption worknote](build-pmd-06-worknote.md) |
