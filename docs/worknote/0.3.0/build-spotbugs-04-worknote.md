...
| Current remediation baseline | 65 reviewed findings: 47 false positives + 18 policy-noise findings; 61 exact selectors; no accepted legacy |
...
| `SB04-018` | `ioc-adapter-ingest` | `NP_NULL_ON_SOME_PATH_FROM_RETURN_VALUE` | `com.iocextractor.adapter.in.ingest.FileSystemSourceLifecycle#fileName` | P2 / `STYLE` | Исходный ternary был безопасен, но повторный analyzer-opaque вызов устранён: nullable leaf сохраняется один раз. `ed71e9161cf3ad0bebb1c31c07c0036f/0` | `resolved-by-hardening` | `NP-HARDEN-01` | `resolved; removed from baseline` |
| `SB04-019` | `ioc-adapter-ingest` | `NP_NULL_ON_SOME_PATH_FROM_RETURN_VALUE` | `com.iocextractor.adapter.in.ingest.FileSystemSourceLifecycle#move` | P2 / `STYLE` | Production wiring отклонял blank directory через `@NotBlank`, но публичный адаптер принимал empty `Path`. Constructor guard и явная parent check теперь закрепляют локальный контракт без raw NPE. `cc923aad27b2cc302d2874fa859096d3/0` | `resolved-by-hardening` | `NP-HARDEN-01` | `resolved; empty-root regression added and baseline entry removed` |
...
| `SB04-033` | `ioc-adapter-transport-smb` | `NP_NULL_ON_SOME_PATH_FROM_RETURN_VALUE` | `com.iocextractor.adapter.out.transport.smb.SmbFileTransport#lambda$localFiles$10` | P2 / `STYLE` | Direct-child proof был корректен, но comparator повторно разыменовывал analyzer-opaque leaf. Явное построение upload plan сохраняет и валидирует leaf один раз. `5e5bfca26b4d0a401aaf36091430e7c0/0` | `resolved-by-hardening` | `SMB-PLAN-01` | `resolved; removed from baseline` |
| `SB04-034` | `ioc-adapter-transport-smb` | `NP_NULL_ON_SOME_PATH_FROM_RETURN_VALUE` | `com.iocextractor.adapter.out.transport.smb.SmbFileTransport#lambda$localFiles$11` | P2 / `STYLE` | `peek` больше не используется как security validator; mapping в immutable upload-plan entry выполняет обязательную leaf validation. `9456a9c7e501bc30df6de8d94d6bc29c/0` | `resolved-by-hardening` | `SMB-PLAN-01` | `resolved; removed from baseline` |
| `SB04-035` | `ioc-adapter-transport-smb` | `NP_NULL_ON_SOME_PATH_FROM_RETURN_VALUE` | `com.iocextractor.adapter.out.transport.smb.SmbFileTransport#publish` | P2 / `STYLE` | Publish больше не восстанавливает leaf из `Path`: он использует заранее проверенные local/remote coordinates из upload plan. `ab4dd7d9826bddec6a3960784b7ae9a1/0` | `resolved-by-hardening` | `SMB-PLAN-01` | `resolved; removed from baseline` |
| `SB04-036` | `ioc-adapter-transport-smb` | `NP_NULL_ON_SOME_PATH_FROM_RETURN_VALUE` | `com.iocextractor.adapter.out.transport.smb.SmbFileTransport#publish` | P2 / `STYLE` | Marker и data entries используют тот же immutable plan; повторного nullable dereference нет. `ab4dd7d9826bddec6a3960784b7ae9a1/1` | `resolved-by-hardening` | `SMB-PLAN-01` | `resolved; removed from baseline` |
| `SB04-037` | `ioc-adapter-transport-smb` | `NP_NULL_ON_SOME_PATH_FROM_RETURN_VALUE` | `com.iocextractor.adapter.out.transport.smb.SmbFileTransport#verifyUploadedSizes` | P2 / `STYLE` | Verification получает exact remote path из той же записи, которая была передана в upload; путь не вычисляется повторно. `588e7a610a503a476d76ef12f69e2911/0` | `resolved-by-hardening` | `SMB-PLAN-01` | `resolved; upload/stat path-identity regression added` |
| `SB04-038` | `ioc-application` | `NP_NULL_ON_SOME_PATH_FROM_RETURN_VALUE` | `com.iocextractor.application.sync.RemoteFetchService#finalPathFor` | P2 / `STYLE` | Existing absolute-inbox + safe-leaf proof remains valid; containment now compares from the non-null inbox side, so a future parentless candidate produces the contextual boundary failure instead of NPE. `b4a1a3c4744e27774c3872fe8e1ac3f7/0` | `resolved-by-hardening` | `NP-HARDEN-01` | `resolved; hostile-name regression added and baseline entry removed` |
| `SB04-039` | `ioc-application` | `NP_NULL_ON_SOME_PATH_FROM_RETURN_VALUE` | `com.iocextractor.application.sync.RemoteFetchService#finalPathFor` | P2 / `STYLE` | The same null-safe containment is applied to the stable suffixed candidate. `b4a1a3c4744e27774c3872fe8e1ac3f7/1` | `resolved-by-hardening` | `NP-HARDEN-01` | `resolved; removed from baseline` |
...
|---|---|---|---|
| `C1-NP-A` | `SB04-017`, `SB04-020`, `SB04-024..028`, `SB04-031..032` | Every dereferenced path is an immediate entry returned by `Files.list(verifiedDirectory)`; later methods receive the same materialized child list | A directory entry has a leaf name. SpotBugs does not carry this NIO stream invariant through lambdas/callers; false positives |
| `C1-NP-B` | `SB04-018..019` | `fileName` handled the nullable result but repeated the call. Production targets had parents under validated daemon configuration, while direct construction still accepted an empty `Path` | Initial production false-positive proof was valid but not self-contained at the adapter boundary; superseded by `NP-HARDEN-01` |
...
| `C1-NP-F` | `SB04-038..039` | Inbox is normalized to an absolute path; remote `leafName` rejects blank, dot and separator-bearing names. Normal and suffixed candidates are therefore direct inbox children | Both findings were false positives under the current contract; null-safe containment and hostile-name regressions now make the proof executable and remove both baseline entries |
| `NP-HARDEN-01` | `SB04-018..019`, `SB04-038..039` | Lifecycle roots are validated inside the public adapter, nullable leaf/parent values are captured once, and inbox containment compares from its non-null trusted root. Parameterized remote-name tests reject dot, parent and trailing-separator forms before download | Four analyzer-opaque nullable flows are hardened and removed from the accepted baseline without changing valid path behavior |
| `SMB-PLAN-01` | `SB04-033..037` | Every regular local child is mapped once to an immutable entry containing its validated leaf and exact temporary remote path. Upload and size verification consume that same entry; regression evidence compares all upload/stat paths | Five analyzer-opaque nullable flows and the `peek`-based validation seam are removed without changing atomic publish ordering |
