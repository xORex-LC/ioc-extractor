...
| Current remediation baseline | 70 reviewed findings: 52 false positives + 18 policy-noise findings; 65 exact selectors; no accepted legacy |
...
| `BL-NIO` | 14 findings in `C1-NP-A` | `NP_*` + exact class/method | `false-positive` | Proven direct-child provenance | Owning path modules | Path provenance changes | `reviewed`; lifecycle and remote-inbox findings were hardened and removed under `NP-HARDEN-01` |
